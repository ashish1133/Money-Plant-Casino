...
document.addEventListener('DOMContentLoaded', () => { setupEventListeners(); initNavToggle(); initRevealObserver(); initTiltEffects(); initFinanceButtons(); initAuthSystem(); initModalTriggers(); initHeroMetrics(); initLiveTicker(); initSpotlightCountdown(); initHeatmapBars(); initCatalogControls(); initPerformanceMode(); bootstrapFromServer(); });
...
function setupEventListeners(){
	...
	// Throttle scroll work to once per animation frame and mark as passive
	let _scrollTick=false; const onScroll=()=>{ if(_scrollTick) return; _scrollTick=true; requestAnimationFrame(()=>{ updateActiveNav(); _scrollTick=false; }); };
...
function initTiltEffects(){
	// Skip tilt on touch/coarse pointers to reduce jank on mobile/tablet
	try { if(!window.matchMedia || !matchMedia('(pointer: fine)').matches) return; } catch(_) {}
	document.querySelectorAll('.tilt-hover').forEach(el=>{
		el.addEventListener('mousemove',e=>{
			const r=el.getBoundingClientRect();
			const x=e.clientX-r.left; const y=e.clientY-r.top;
			const rotX=((y/r.height)-0.5)*-10; const rotY=((x/r.width)-0.5)*10;
			el.style.transform=`perspective(800px) translateY(-8px) rotateX(${rotX}deg) rotateY(${rotY}deg)`;
		}, { passive: true });
		el.addEventListener('mouseleave',()=>{ el.style.transform=''; });
	});
}

// Performance mode toggle (reduces visual effects for low-end devices)
function initPerformanceMode(){
	...
	// Add a toggle button into the navbar controls if available
	const host = document.querySelector('.toggle-group') || document.querySelector('.user-section');
	if(host && !document.getElementById('perfModeBtn')){
		const btn=document.createElement('button');
		btn.id='perfModeBtn';
		btn.type='button';
		btn.className='icon-btn';
		const setLabel=()=>{ btn.textContent = (enabled? '⚡ Performance: On' : '⚡ Performance: Off'); };
		setLabel();
		btn.addEventListener('click', ()=>{
			...
			document.documentElement.classList.toggle('perf-mode', enabled);
			setLabel();
			try{ showNotification('Performance Mode', enabled?'Reduced effects enabled':'Full effects restored', 'info'); }catch(_){ }
		});
		host.prepend(btn);
	}
}
